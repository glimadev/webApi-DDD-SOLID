	...
	angular
	  ...
	  .controller('MwlCalendarCtrl', ["$scope", "$log", "$timeout", "$attrs", "$locale", "moment", "calendarTitle", function($scope, $log, $timeout, $attrs, $locale, moment, calendarTitle) {
	    ...
	    function eventIsValid(event) {
	      if (!event.dateStart) {
	        $log.warn('Bootstrap calendar: ', 'Event is missing the dateStart field', event);
	      }

	      if (!angular.isDate(event.dateStart)) {
	        $log.warn('Bootstrap calendar: ', 'Event dateStart should be a javascript date object', event);
	      }

	      if (angular.isDefined(event.dateEnd)) {
	        if (!angular.isDate(event.dateEnd)) {
	          $log.warn('Bootstrap calendar: ', 'Event dateEnd should be a javascript date object', event);
	        }
	        if (moment(event.dateStart).isAfter(moment(event.dateEnd))) {
	...
	angular
	  ...
	  .controller('MwlCalendarMonthCtrl', ["$scope", "moment", "calendarHelper", "calendarConfig", function($scope, moment, calendarHelper, calendarConfig) {
	    ...
	    vm.handleEventDrop = function(event, newDayDate) {
	      ...
	      var newStart = moment(event.dateStart)
	...
	angular
	  ...
	  .controller('MwlCalendarWeekCtrl', ["$scope", "$sce", "moment", "calendarHelper", "calendarConfig", function($scope, $sce, moment, calendarHelper, calendarConfig) {
	    ...
	    vm.weekDragged = function(event, daysDiff, minuteChunksMoved) {
	      ...
	      var newStart = moment(event.dateStart).add(daysDiff, 'days');
	      var newEnd = moment(event.dateEnd).add(daysDiff, 'days');
	...
	angular
	  ...
	  .factory('calendarHelper', ["dateFilter", "moment", "calendarConfig", function(dateFilter, moment, calendarConfig) {
	    ...
	    function eventIsInPeriod(event, periodStart, periodEnd) {
	      ...
	      var eventStart = moment(event.dateStart);
	      var eventEnd = moment(event.dateEnd || event.dateStart);
	      ...
	      if (angular.isDefined(event.recursOn)) {
	        ...
	        eventEnd = adjustEndDateFromStartDiff(event.dateStart, eventStart, eventEnd);
	    ...
	    function getWeekView(events, currentDay) {
	      ...
	      var eventsSorted = filterEventsInPeriod(events, startOfWeek, endOfWeek).map(function(event) {
	        ...
	        var eventStart = moment(event.dateStart).startOf('day');
	        var eventEnd = moment(event.dateEnd || event.dateStart).startOf('day');
	    ...
	    function getDayView(events, currentDay, dayViewStart, dayViewEnd, dayViewSplit) {
	      ...
	      return eventsInPeriod.map(function(event) {
	        if (moment(event.dateStart).isBefore(calendarStart)) {
	          event.top = 0;
	        } else {
	          event.top = (moment(event.dateStart).startOf('minute').diff(calendarStart.startOf('minute'), 'minutes') * hourHeightMultiplier) - 2;
	        }

	        if (moment(event.dateEnd || event.dateStart).isAfter(calendarEnd)) {
	          event.height = calendarHeight - event.top;
	        } else {
	          var diffStart = event.dateStart;
	          if (moment(event.dateStart).isBefore(calendarStart)) {
	          ...
	          if (!event.dateEnd) {
	            ...
	            event.height = moment(event.dateEnd || event.dateStart).diff(diffStart, 'minutes') * hourHeightMultiplier;
	          }
	        }
	        ...
	        buckets.forEach(function(bucket, bucketIndex) {
	          ...
	          bucket.forEach(function(bucketItem) {
	            if (eventIsInPeriod(event, bucketItem.dateStart, bucketItem.dateEnd || bucketItem.dateStart) ||
	              eventIsInPeriod(bucketItem, event.dateStart, event.dateEnd || event.dateStart)) {
	    ...
	    function getWeekViewWithTimes(events, currentDay, dayViewStart, dayViewEnd, dayViewSplit) {
	      ...
	      weekView.days.forEach(function(day) {
	        var dayEvents = weekView.events.filter(function(event) {
	          return moment(event.dateStart).startOf('day').isSame(moment(day.date).startOf('day'));
	        });
